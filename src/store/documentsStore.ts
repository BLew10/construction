...
interface DocumentsState {
  ...
  uploadDocument: (
    document: Omit<
      Document,
      "id" | "uploadedAt" | "version" | "aiProcessed" | "commentCount"
    >
  ) => Promise<void>;
...
export const useDocumentsStore = create<DocumentsState>((set, get) => ({
    ...
    try {
      ...
      const mockDocuments: Document[] = [
        // Project 1 Documents
        {
          ...
          aiProcessed: true,
          aiSummary:
            "Complete floor plans for all 3 levels including dimensions and room layouts.",
        ...
        },
        {
          ...
          aiProcessed: true,
          aiSummary:
            "Official building permit #12345 issued by City Planning Department on Sept 10, 2023.",
        ...
        // Project 2 Documents
        {
          ...
          aiProcessed: true,
          aiSummary:
            "Detailed electrical wiring diagrams for all building systems including lighting, power, and low voltage.",
      ...
      set({ documents: projectId ? filteredDocuments : mockDocuments, isLoading: false });
    } catch (error) {
  ...
  fetchDocument: async (documentId: string) => {
    ...
    try {
      ...
      if (!document) {
        ...
        const potentiallyLoadedDoc = get().documents.find(
  ...
  processDocumentWithAI: async (documentId: string) => {
    ...
    try {
      ...
      const aiSummary =
        "This document contains information about the project specifications and requirements.";
      const aiKeywords = [
        "construction",
        "specifications",
        "requirements",
      ...
      const mockComments: DocumentComment[] = [
        {
          ...
          content:
            "Please review the door dimensions on page 3. They seem inconsistent with the architectural specifications.",
        ...
        },
        {
          ...
          content:
            "The electrical load calculations need to be updated based on the revised equipment schedule.",
    ...
    try {
      ...
      set((state) => {
        ...
        Object.keys(updatedComments).forEach((documentId) => {
          updatedComments[documentId] = updatedComments[documentId].map(
            (comment) =>
  ...
  deleteComment: async (commentId) => {
    ...
    try {
      ...
      set((state) => {
        ...
        const updatedDocuments = documentId
  ...
  updateDocument: async (id, updates) => {
    ...
    try {
      ...
      set((state) => {
        ...
        // Check if the updated document is the current one and update it too
        ...
        return {
          documents: updatedDocuments,
          currentDocument: updatedCurrentDocument, // Update currentDocument state
          isLoading: false, // Set loading false after updates
        };
      });
    } catch (error) {
